...
#include <algorithm>			// для нахождения min и max
#include <fstream>
using namespace std;
...
public:
	...
	Iterator<T> last() { return Iterator<T>(nullptr); }     // возвращает указатель на последний элемент
	size_t give_size() { return size; }						// возвращает размер списка
};
